...
export function gameSequence(seed: number, count: number): GameDescriptor[] {
  ...
  for (const category of CATEGORIES) {
    ...
    if (categoryGames.length === 0) {
      throw new Error(`registry is missing ${category}`);
    }
...
export function parseEvidence(value: unknown): MiniGameEvidence[] {
  ...
  return value.map((raw, index) => {
    ...
    if (typeof gameId !== "string" || gameId.length === 0) {
      throw new Error(`evidence[${index}].gameId is invalid`);
    }
    if (!finiteNumber(gameIndex) || !Number.isInteger(gameIndex)) {
      throw new Error(`evidence[${index}].gameIndex is invalid`);
    }
    if (!finiteNumber(evidenceSeed) || !Number.isInteger(evidenceSeed)) {
      throw new Error(`evidence[${index}].gameSeed is invalid`);
    }
    if (!finiteNumber(score) || !Number.isInteger(score)) {
      throw new Error(`evidence[${index}].score is invalid`);
    }
    if (!finiteNumber(accuracy)) {
      throw new Error(`evidence[${index}].accuracy is invalid`);
    }
    if (!finiteNumber(mistakes) || !Number.isInteger(mistakes)) {
      throw new Error(`evidence[${index}].mistakes is invalid`);
    }
    if (!finiteNumber(durationMs) || !Number.isInteger(durationMs)) {
      throw new Error(`evidence[${index}].durationMs is invalid`);
    }

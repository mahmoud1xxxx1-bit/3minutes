...
export const settleRankedMatchV2 = onCall(CALLABLE_OPTIONS, async (request) => {
  ...
  return db.runTransaction(async (transaction) => {
    ...
    let wagerGold;
    try {
      wagerGold = parseGoldWager(match.wagerGold);
    } catch (error) {
      throw new HttpsError(
        "data-loss",
        error instanceof Error ? error.message : "Invalid ranked Gold wager.",
      );
    }
    ...
    const exactSuccessfulDraw =
      outcome === "tie" &&
      bothComplete &&
      progressA.totalScore === perfectScore &&
      progressB.totalScore === perfectScore;
    const goldSettlement = settleGoldEscrow({
      wagerGold,
      outcome,
      tieIsFailure: !exactSuccessfulDraw,
    });
    ...
    const goldCreditA = goldSettlement.playerARefund + goldSettlement.playerAPayout;
    const goldCreditB = goldSettlement.playerBRefund + goldSettlement.playerBPayout;
    const nextGoldA = Math.max(0, intValue(inventoryA.gold)) + goldCreditA;
    const nextGoldB = Math.max(0, intValue(inventoryB.gold)) + goldCreditB;
    ...
    const payload = {
      ...
      playerA: settlementPlayer({
        ...
        wagerGold,
        goldCredited: goldCreditA,
        goldNetDelta: goldCreditA - wagerGold,
        goldBalanceAfter: nextGoldA,
      }),
      playerB: settlementPlayer({
        ...
        wagerGold,
        goldCredited: goldCreditB,
        goldNetDelta: goldCreditB - wagerGold,
        goldBalanceAfter: nextGoldB,
      }),
    ...
    transaction.create(
      db.collection(COLLECTIONS.goldTransactions).doc(`${matchId}_${playerAId}_settlement`),
      {
        uid: playerAId,
        matchId,
        seasonId,
        wagerGold,
        reason: goldSettlement.reason,
        amount: goldCreditA,
        netMatchDelta: goldCreditA - wagerGold,
        balanceAfter: nextGoldA,
        createdAt: Timestamp.fromDate(settledAt),
      },
    );
    transaction.create(
      db.collection(COLLECTIONS.goldTransactions).doc(`${matchId}_${playerBId}_settlement`),
      {
        uid: playerBId,
        matchId,
        seasonId,
        ...
        reason: goldSettlement.reason,
        amount: goldCreditB,
        netMatchDelta: goldCreditB - wagerGold,
    ...
    transaction.update(matchRef, {
      ...
      goldEscrowStatus: "settled",
      goldEscrowReason: goldSettlement.reason,
      burnedGold: goldSettlement.burnedGold,
      updatedAt: FieldValue.serverTimestamp(),
    });

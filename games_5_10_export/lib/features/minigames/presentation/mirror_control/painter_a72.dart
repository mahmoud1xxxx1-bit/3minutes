...
class GamePainterA72 extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < 150; i++) {
      ...
      canvas.drawCircle(Offset(px, py), rnd.nextDouble() * 15 + 5, Paint()..color = Colors.white.withOpacity(0.1)..maskFilter = const MaskFilter.blur(BlurStyle.normal, 5));
    }
    ...
    for (int i = 0; i < engine.targets.length; i++) {
        ...
        if (isActive) {
            ...
            canvas.drawCircle(target, 35*p, Paint()..color=Colors.lightBlueAccent.withOpacity(0.6)..maskFilter=const MaskFilter.blur(BlurStyle.normal, 3));

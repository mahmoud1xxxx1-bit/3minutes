...
class GamePainterA22Factory extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Slime puddles
    final slimePaint = Paint()..color = const Color(0xFF32CD32).withOpacity(0.4)..maskFilter = const MaskFilter.blur(BlurStyle.normal, 3);
    for (int i = 0; i < 5; i++) {
    ...
    for (int i = 0; i < engine.targets.length; i++) {
      ...
      if (isNext) {
        final glowPaint = Paint()
          ..color = const Color(0xFF39FF14).withOpacity(0.6) // Neon green
    ...
    if (engine.exitGate != null) {
      ...
      final sprayPaint = Paint()..color = Colors.white.withOpacity(0.5)..maskFilter = const MaskFilter.blur(BlurStyle.normal, 2.0);
      for(int i=0; i<3; i++) {

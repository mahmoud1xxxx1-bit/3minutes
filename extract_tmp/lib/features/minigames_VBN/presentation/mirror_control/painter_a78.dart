...
class GamePainterA78 extends CustomPainter {
  GamePainterA78({required this.engine, this.images});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final rnd = math.Random(78);

    // ==========================================
    // 1. ARTISTIC BACKGROUND AND FLOOR
    // ==========================================
    canvas.drawRect(const Rect.fromLTWH(0, 0, GameEngine.fieldSize, GameEngine.fieldSize), Paint()..color = Colors.grey.shade800);
    // Green ooze rivers
    canvas.drawPath(Path()..moveTo(0, 100)..quadraticBezierTo(GameEngine.fieldSize/2, 200, GameEngine.fieldSize, 100)
        ..lineTo(GameEngine.fieldSize, 150)..quadraticBezierTo(GameEngine.fieldSize/2, 250, 0, 150)..close(),
        Paint()..color = Colors.greenAccent.withOpacity(0.6));
    // Grate floor
    final gratePaint = Paint()..color = Colors.black38..style = PaintingStyle.stroke..strokeWidth = 2;
    for(double x=0; x<GameEngine.fieldSize; x+=60) {
        canvas.drawLine(Offset(x, 0), Offset(x, GameEngine.fieldSize), gratePaint);
        canvas.drawLine(Offset(0, x), Offset(GameEngine.fieldSize, x), gratePaint);
    }
    ...
    for (int i = 0; i < engine.obstacles.length; i++) {
        ...
        canvas.drawRect(obs, Paint()..color=Colors.brown.shade600); 
        // Rusted barrel details
        canvas.drawCircle(Offset(obs.center.dx, obs.center.dy), obs.width/2 - 2, Paint()..color=Colors.orange.shade800);
        canvas.drawCircle(Offset(obs.center.dx, obs.center.dy), obs.width/2 - 6, Paint()..color=Colors.greenAccent.withOpacity(0.8));
    }
    ...
    for (int i = 0; i < engine.targets.length; i++) {
        ...
        // Toxic glowing orb
        canvas.drawCircle(target, 15, Paint()..color=Colors.lightGreenAccent);
  ...
  @override bool shouldRepaint(covariant GamePainterA78 old) => true;
}

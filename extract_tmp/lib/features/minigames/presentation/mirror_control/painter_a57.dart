...
class GamePainterA57 extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawPath(path, Paint()..color = const Color(0xFF1F3821).withOpacity(0.3)..style = PaintingStyle.stroke..strokeWidth = 20);
    for (int i = 0; i < 50; i++) {
      ...
      canvas.drawCircle(Offset(x, y), 2, Paint()..color = Colors.lightGreenAccent.withOpacity(0.4));
    }
    for (int i = 0; i < engine.obstacles.length; i++) {
        final obs = engine.obstacles[i];
        canvas.drawRect(obs.shift(const Offset(5, 15)), Paint()..color=Colors.black.withOpacity(0.7));
    ...
    for (int i = 0; i < engine.targets.length; i++) {
        ...
        if (isActive) {
            ...
            canvas.drawCircle(target, 35*p, Paint()..color=Colors.amberAccent.withOpacity(0.6)..maskFilter=const MaskFilter.blur(BlurStyle.normal, 3));

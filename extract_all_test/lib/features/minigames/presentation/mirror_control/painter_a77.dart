...
class GamePainterA77 extends CustomPainter {
  GamePainterA77({required this.engine, this.images});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Dark stone tiles and gold veins
    for (double i = 0; i < GameEngine.fieldSize; i+=60) {
      for (double j = 0; j < GameEngine.fieldSize; j+=60) {
        canvas.drawRect(Rect.fromLTWH(i, j, 58, 58), Paint()..color = Colors.grey.shade900);
        if (rnd.nextDouble() > 0.7) {
            canvas.drawLine(Offset(i, j), Offset(i + 58, j + 58), Paint()..color=Colors.amberAccent..strokeWidth=2);
        }
      }
    }
    // Rails
    for (double i = 0; i < GameEngine.fieldSize; i+=80) {
        canvas.drawLine(Offset(i, 0), Offset(i, GameEngine.fieldSize), Paint()..color=Colors.brown.shade700..strokeWidth=4);
    }
    ...
    for (int i = 0; i < engine.obstacles.length; i++) {
        ...
        canvas.drawRect(obs, Paint()..color=Colors.brown.shade900);
        // Gold nuggets
        canvas.drawCircle(Offset(obs.center.dx, obs.center.dy), 10, Paint()..color=Colors.amber.shade300);
    }
    ...
    for (int i = 0; i < engine.targets.length; i++) {
        ...
        // Aztec sun emblem
        canvas.drawCircle(target, 18, Paint()..color=Colors.yellow.shade700);
        for(int a=0; a<8; a++) {
            double angle = a * math.pi / 4;
            canvas.drawCircle(target + Offset(math.cos(angle)*15, math.sin(angle)*15), 5, Paint()..color=Colors.orange);
        }
  ...
  @override bool shouldRepaint(covariant GamePainterA77 old) => true;
}

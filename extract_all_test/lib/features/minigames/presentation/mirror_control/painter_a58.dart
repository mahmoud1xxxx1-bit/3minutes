...
class GamePainterA58 extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for(int i = 0; i < 15; i++) {
        canvas.drawCircle(Offset(rnd.nextDouble() * GameEngine.fieldSize, rnd.nextDouble() * GameEngine.fieldSize), rnd.nextDouble() * 100 + 50, Paint()..color = const Color(0xFF14304D).withOpacity(0.3)..maskFilter=const MaskFilter.blur(BlurStyle.normal, 3));
    }
    for (int i = 0; i < engine.obstacles.length; i++) {
        final obs = engine.obstacles[i];
        canvas.drawRect(obs.shift(const Offset(0, 15)), Paint()..color=Colors.black.withOpacity(0.8));

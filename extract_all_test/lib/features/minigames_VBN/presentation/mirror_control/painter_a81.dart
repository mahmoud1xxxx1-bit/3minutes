...
class GamePainterA81 extends CustomPainter {
  GamePainterA81({required this.engine, this.images});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final rnd = math.Random(81);

    // ==========================================
    // 1. ARTISTIC BACKGROUND AND FLOOR
    // ==========================================
    final bgPaint = Paint()..color = const Color(0xFFD2B48C);
    canvas.drawRect(const Rect.fromLTWH(0, 0, GameEngine.fieldSize, GameEngine.fieldSize), bgPaint);
    
    final plankPaint = Paint()..color = const Color(0xFFC19A6B)..style = PaintingStyle.stroke..strokeWidth = 2;
    for (double x = 0; x < GameEngine.fieldSize; x+=100) {
      canvas.drawLine(Offset(x, 0), Offset(x, GameEngine.fieldSize), plankPaint);
    }
    for (double y = 0; y < GameEngine.fieldSize; y+=40) {
      canvas.drawLine(Offset(0, y), Offset(GameEngine.fieldSize, y), plankPaint);
    }
    ...
    for (int i = 0; i < engine.obstacles.length; i++) {
        ...
        final blockColors = [Colors.redAccent, Colors.blueAccent, Colors.greenAccent, Colors.amber];
        final color = blockColors[i % blockColors.length];
        
        canvas.drawRect(obs, Paint()..color=color);
        canvas.drawRect(obs, Paint()..color=Colors.black26..style=PaintingStyle.stroke..strokeWidth=4);
        
        canvas.drawCircle(obs.center, math.min(obs.width, obs.height) * 0.3, Paint()..color=Colors.white.withOpacity(0.3));
    }
    ...
    for (int i = 0; i < engine.targets.length; i++) {
        ...
        final keyPaint = Paint()..color = Colors.grey[700]!..style = PaintingStyle.fill;
        canvas.drawRect(const Rect.fromLTWH(-5, -20, 10, 40), keyPaint);
        canvas.drawCircle(const Offset(0, -25), 12, Paint()..color=Colors.grey[700]!..style=PaintingStyle.stroke..strokeWidth=6);
  ...
  @override bool shouldRepaint(covariant GamePainterA81 old) => true;
}

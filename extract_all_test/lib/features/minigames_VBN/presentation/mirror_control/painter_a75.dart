...
class GamePainterA75 extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (double y = 0; y < GameEngine.fieldSize; y+=40) {
      ...
      canvas.drawLine(Offset(0, y), Offset(GameEngine.fieldSize, y), Paint()..color=const Color(0xFFE91E63).withOpacity(0.2 + 0.3*intensity));
    }
    for (double x = 0; x < GameEngine.fieldSize; x+=40) {
      canvas.drawLine(Offset(x, 0), Offset(x, GameEngine.fieldSize), Paint()..color=const Color(0xFF00BCD4).withOpacity(0.2));
    }
    ...
    for (int i = 0; i < 5; i++) {
      ...
      canvas.drawRect(Rect.fromCenter(center: Offset(cx, cy), width: 100, height: 50), Paint()..color=Colors.cyanAccent.withOpacity(0.1)..maskFilter=const MaskFilter.blur(BlurStyle.normal, 3));
      canvas.drawRect(Rect.fromCenter(center: Offset(cx, cy), width: 100, height: 50), Paint()..color=Colors.cyanAccent.withOpacity(0.3)..style=PaintingStyle.stroke..strokeWidth=2);
    }
    ...
    for (int i = 0; i < 200; i++) {
      ...
      canvas.drawRect(Rect.fromLTWH(px, py, 2, 10), Paint()..color = Colors.greenAccent.withOpacity(0.6));
    }
    ...
    for (int i = 0; i < engine.targets.length; i++) {
        ...
        if (isActive) {
            ...
            canvas.drawCircle(target, 35*p, Paint()..color=Colors.greenAccent.withOpacity(0.6)..maskFilter=const MaskFilter.blur(BlurStyle.normal, 3));

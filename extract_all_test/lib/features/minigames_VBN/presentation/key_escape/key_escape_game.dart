...
class _KeyEscapeGameState extends State<KeyEscapeGame> {
  ...
  void _nextPuzzle() {
    setState(() {
      _puzzleInStage++;
      if (_puzzleInStage >= 3) {
        widget.onComplete(const MiniGameResult(
          completed: true,
          score: 100,
          accuracy: 1.0,
          mistakes: 0,
  ...
  void _onPanStart(DragStartDetails details, PuzzleBlock block, double cellSize) {
    ...
    if (block.isHorizontal) {
      ...
      for (var other in _blocks) {
        ...
        bool overlapY = (block.y < other.y + (other.isHorizontal ? 1 : other.length)) && (block.y + 1 > other.y);
        if (overlapY) {
          if (other.x < block.x) {
            _minDrag = math.max(_minDrag, other.x + (other.isHorizontal ? other.length : 1).toDouble());
          } else {
      ...
      for (var other in _blocks) {
        if (other.id == block.id) continue;
        bool overlapX = (block.x < other.x + (other.isHorizontal ? other.length : 1)) && (block.x + 1 > other.x);
        if (overlapX) {
          if (other.y < block.y) {
            _minDrag = math.max(_minDrag, other.y + (other.isHorizontal ? 1 : other.length).toDouble());
          } else {
  ...
  void _onPanUpdate(DragUpdateDetails details, double cellSize) {
    ...
    setState(() {
      if (_draggingBlock!.isHorizontal) {
        ...
        _draggingBlock!.logicalX = _draggingBlock!.logicalX.clamp(_minDrag, _maxDrag);
      } else {
        _draggingBlock!.logicalY += details.delta.dy / cellSize;
        _draggingBlock!.logicalY = _draggingBlock!.logicalY.clamp(_minDrag, _maxDrag);
      }
    });
  }
  ...
  void _onPanEnd(DragEndDetails details) {
    ...
    setState(() {
      ...
      if (_draggingBlock!.isTarget && _draggingBlock!.logicalX >= 5.0) {
        _isRoundOver = true;
        _roundWon = true;
        try { MinigameEnvironment.of(context).playSuccess(const Offset(200, 200)); } catch (e) {}
        Future.delayed(const Duration(seconds: 1), _nextPuzzle);
      } else if (_mistakes >= 3) {
        // Failed the puzzle
        _isRoundOver = true;
        _roundWon = false;
        try { MinigameEnvironment.of(context).playError(const Offset(200, 200)); } catch (e) {}
  ...
  Widget build(BuildContext context) {
    return Stack(
      children: [
        Positioned.fill(
          child: CustomPaint(painter: _MagicBackgroundPainter()),
        ),
        Center(
          child: Column(
            ...
            children: [
              ...
              Container(
                ...
                child: Row(
                  ...
                  children: [
                    Column(
                      children: [
                        const Text('MOVES', style: TextStyle(color: Colors.white70, fontSize: 14)),
                        Text('$_moves / $_targetMoves', style: const TextStyle(color: Colors.white, fontSize: 24, fontWeight: FontWeight.bold)),
                      ],
                    ),
                    const SizedBox(width: 40),
                    Column(
                      children: [
                        const Text('MISTAKES', style: TextStyle(color: Colors.redAccent, fontSize: 14)),
                        Row(
                          children: List.generate(3, (index) => Icon(
                            Icons.close,
                            color: index < _mistakes ? Colors.red : Colors.white24,
                            size: 28,
                          )),
                        )
                      ],
              ...
              LayoutBuilder(
                builder: (context, constraints) {
                  double boardSize = math.min(constraints.maxWidth * 0.9, 450);
                          ...
                          Positioned(
                            ...
                            child: Container(
                              decoration: BoxDecoration(
                                ...
                                boxShadow: const [BoxShadow(color: Colors.yellowAccent, blurRadius: 20)],
                              ),
                              child: const Center(child: Icon(Icons.exit_to_app, color: Colors.yellowAccent, size: 20)),
                            ),
                          ),
                                ...
                                child: Center(
                                  child: Text(
                                    ...
                                    style: TextStyle(
                                      fontSize: 54,
                                      fontWeight: FontWeight.w900,
                                      color: _roundWon ? Colors.greenAccent : Colors.redAccent,
...
class _GridPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    Paint paint = Paint()..color = Colors.white.withOpacity(0.05)..strokeWidth = 2;
    for (int i=1; i<6; i++) {
...
class _GlossyBlockPainter extends CustomPainter {
  ...
  _GlossyBlockPainter({required this.isHorizontal, required this.isTarget, required this.length});

  @override
  void paint(Canvas canvas, Size size) {
    final RRect rrect = RRect.fromRectAndRadius(Offset.zero & size, const Radius.circular(12));
    
    // Drop Shadow
    canvas.drawRRect(rrect.shift(const Offset(3, 5)), Paint()..color = Colors.black.withOpacity(0.6)..maskFilter = const MaskFilter.blur(BlurStyle.normal, 6));
    ...
    final Path innerPath = Path()..addRRect(RRect.fromRectAndRadius(Rect.fromLTWH(4, 4, size.width - 8, size.height - 8), const Radius.circular(8)));
    final Path bevel = Path.combine(PathOperation.difference, borderPath, innerPath);
    
    canvas.drawPath(bevel, Paint()
      ..shader = LinearGradient(
        begin: Alignment.topLeft, end: Alignment.bottomRight,
        colors: [Colors.white.withOpacity(0.6), Colors.black.withOpacity(0.3)]
      ).createShader(Offset.zero & size)
    );

    final Path highlight = Path()
      ..addRRect(RRect.fromRectAndRadius(Rect.fromLTWH(6, 6, size.width - 12, size.height * 0.4), const Radius.circular(6)));
    ...
    Paint detailPaint = Paint()..color = Colors.white.withOpacity(0.3)..style = PaintingStyle.stroke..strokeWidth = 3;
    if (isTarget) {
      TextPainter tp = TextPainter(text: const TextSpan(text: '🗝️', style: TextStyle(fontSize: 32, shadows: [Shadow(color: Colors.black45, blurRadius: 4, offset: Offset(2,2))])), textDirection: TextDirection.ltr);
      tp.layout();
      ...
      if (isHorizontal) {
        for (int i=1; i<length * 2; i++) {
          ...
          canvas.drawLine(Offset(x, size.height * 0.2), Offset(x, size.height * 0.8), detailPaint);
        }
      } else {
        for (int i=1; i<length * 2; i++) {
          double y = i * (size.height / (length * 2));
          canvas.drawLine(Offset(size.width * 0.2, y), Offset(size.width * 0.8, y), detailPaint);
        }
      }
    }
  }
...
class _MagicBackgroundPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i=0; i<100; i++) {
      ...
      canvas.drawCircle(Offset(x, y), s, starPaint..color = Colors.white.withOpacity(r.nextDouble() * 0.5 + 0.1));
    }
  }
  @override
  bool shouldRepaint(covariant CustomPainter oldDelegate) => false;

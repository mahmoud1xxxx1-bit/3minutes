...
class _OnetConnectGameState extends State<OnetConnectGame> with TickerProviderStateMixin {
  ...
  void _generateGrid() {
    ...
    List<int> animals = [];
    int numAnimals = 25; // BACK TO 25! We will handle this in _buildTile
                ...
                Container(
                  ...
                  child: Column(
                    children: [
                      ...
                      Text('\', style: const TextStyle(color: Colors.white, fontSize: 24, fontWeight: FontWeight.bold)),
                    ],
                  ),
                ),
  ...
  Widget _buildTile(TileModel t, double tileWidth, double tileHeight) {
    ...
    if (t.animal > 15) {
      ...
      // Shift hue dynamically based on animal ID
      // Using BlendMode.hue with bright colors forces a hue shift
      // This preserves white background and black outlines perfectly!
      ...
      colorFilter = ColorFilter.mode(
        shiftColors[t.animal % shiftColors.length], 
        BlendMode.hue,
      );
    }

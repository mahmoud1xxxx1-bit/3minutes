...
class GradientBuilder {
  ...
  ui.Shader build() {
    if (isRadial) {
      // Approximation for HTML Canvas radial gradient
...
class HtmlCanvas {
  ...
  set fillStyle(dynamic value) {
    if (value is String) {
      ...
      _fillPaint.color = parseColor(value).withOpacity(globalAlpha);
  ...
  set strokeStyle(dynamic value) {
    if (value is String) {
      ...
      _strokePaint.color = parseColor(value).withOpacity(globalAlpha);
    } else if (value is GradientBuilder) {
  ...
  void fillText(String text, num x, num y) {
    ...
    final span = TextSpan(style: TextStyle(color: _fillPaint.color, fontSize: fontSize, fontWeight: font.contains('bold') ? FontWeight.bold : FontWeight.normal), text: text);

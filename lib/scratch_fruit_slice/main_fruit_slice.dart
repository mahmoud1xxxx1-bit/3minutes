...
class FruitSliceGame extends StatefulWidget {
  const FruitSliceGame({Key? key}) : super(key: key);
  @override
  _FruitSliceGameState createState() => _FruitSliceGameState();
}
...
class _FruitSliceGameState extends State<FruitSliceGame> with SingleTickerProviderStateMixin {
  ...
  void _update(Duration elapsed) {
    ...
    setState(() {
      ...
      if (spawnTimer <= 0) {
        _spawnWave(size);
        spawnTimer = 1.0 + _rnd.nextDouble() * 2.0; 
      }
      ...
      for (int i = activeItems.length - 1; i >= 0; i--) {
        ...
        item.vy += gravity * dt * timeScale;
        item.rotation += item.vr * dt * timeScale;
        
        if (item.y > size.height + 100 && item.vy > 0) {
          // User request: Missing fruits does NOT end the game. Only breaking plates.
      ...
      for (int i = floatingTexts.length - 1; i >= 0; i--) {
        ...
        floatingTexts[i].y -= 30 * dt; // float upwards
      ...
      for (int i = swipePoints.length - 1; i >= 0; i--) {
        ...
        if (swipePoints[i].age > 0.15) swipePoints.removeAt(i); // slightly faster fade
      }
    });
  }

  void _spawnWave(Size size) {
    int numItems = 1 + _rnd.nextInt(3); 
    for (int i = 0; i < numItems; i++) {
      double startX = size.width * 0.2 + _rnd.nextDouble() * (size.width * 0.6);
      double startY = size.height + 50;
      double vx = (size.width / 2 - startX) * 1.2 + (_rnd.nextDouble() - 0.5) * 200;
      
      double targetY = size.height * 0.1 + _rnd.nextDouble() * (size.height * 0.3);
      double dropDistance = startY - targetY;
      double vy = -sqrt(2 * gravity * dropDistance);
      
      ItemType type = ItemType.values[_rnd.nextInt(4)]; // Normal fruits
      
      if (score > 20 && _rnd.nextDouble() < 0.15) {
  ...
  void _onPanEnd(DragEndDetails details) {
    // When finger is lifted, if they had a small combo, finish it immediately
        ...
        child: Transform.translate(
          ...
          child: Stack(
            children: [
              ...
              // Juice Splatters on the wall!
              for (var s in splatters)
                Positioned(
                  left: s.x - s.size/2, top: s.y - s.size/2,
                  child: Opacity(
                    opacity: (s.life / 5.0).clamp(0.0, 1.0) * 0.6, // fade out over 5s, max opacity 0.6
                    child: Container(
              ...
              Positioned.fill(
                child: CustomPaint(
                  painter: SwipePainter(swipePoints),
                ),
              ),

              // Red Flash for Screen Shake (Bomb hit)
              if (screenShakeTimer > 0)
                Positioned.fill(
                  child: Container(
              ...
              if (!isStarted)
                Center(
                  child: Column(
                    ...
                    children: [
                      const Text("NINJA SLICE", style: TextStyle(fontSize: 60, color: Colors.cyanAccent, fontWeight: FontWeight.bold, letterSpacing: 4, shadows: [Shadow(color: Colors.cyan, blurRadius: 20)])),
                      const SizedBox(height: 40),
                      ElevatedButton(
                        onPressed: _startGame,
                        style: ElevatedButton.styleFrom(backgroundColor: Colors.pinkAccent, padding: const EdgeInsets.symmetric(horizontal: 40, vertical: 20)),
                        child: const Text("START GAME", style: TextStyle(fontSize: 24, color: Colors.white, fontWeight: FontWeight.bold)),
                      )
                    ],
                  ),
                ),
              ...
              if (isGameOver)
                Center(
                  child: Column(
                    mainAxisSize: MainAxisSize.min,
                    children: [
                      const Text("GAME OVER!", style: TextStyle(fontSize: 60, color: Colors.redAccent, fontWeight: FontWeight.bold, letterSpacing: 4, shadows: [Shadow(color: Colors.red, blurRadius: 20)])),
                      const SizedBox(height: 20),
                      Text("FINAL SCORE: $score", style: const TextStyle(fontSize: 30, color: Colors.white)),
                      const SizedBox(height: 40),
                      ElevatedButton(
                        onPressed: _startGame,
                        ...
                        child: const Text("TRY AGAIN", style: TextStyle(fontSize: 24, color: Colors.black, fontWeight: FontWeight.bold)),
                      )
                    ],
                  ),
                ),
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < points.length - 1; i++) {
      ...
      Paint glowPaint = Paint()
        ..color = Colors.cyanAccent.withOpacity(life * 0.6)
        ..strokeWidth = 20.0 * life
        ..strokeCap = StrokeCap.round;
        
      Paint corePaint = Paint()
        ..color = Colors.white.withOpacity(life)
        ..strokeWidth = 6.0 * life
        ..strokeCap = StrokeCap.round;

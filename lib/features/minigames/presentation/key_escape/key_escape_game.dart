...
class _KeyEscapeGameState extends State<KeyEscapeGame> {
  ...
  int get _roundHearts => math.max(0, 2 - _mistakes);
  ...
  void _loadPuzzle() {
    setState(() {
      _currentLevel = (_baseLevel + _puzzleInStage + 1) % 90;
  ...
  void _nextPuzzle() {
    setState(() {
      _puzzleInStage++;
      if (_puzzleInStage >= 2) { // 2 rounds total
        _watch.stop();
        widget.onComplete(
          MiniGameResult(
            completed: true,
  ...
  void _onPanEnd(DragEndDetails details) {
    ...
    setState(() {
      ...
      int currentHearts = math.max(0, 2 - _mistakes);

      if (_draggingBlock!.isTarget && _draggingBlock!.logicalX >= 5.0) {
        _isRoundOver = true;
        _roundWon = true;
        _totalScore += currentHearts * 250;
        try {
      ...
      children: [
        ...
        Center(
          child: Column(
            ...
            children: [
              HeartsDisplay(maxHearts: 2, currentHearts: _roundHearts),
              const SizedBox(height: 10),
              Text(
                'Round ${_puzzleInStage + 1} / 2',

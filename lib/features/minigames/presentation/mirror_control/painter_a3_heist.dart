...
class GamePainterA3Heist extends CustomPainter {
  GamePainterA3Heist({required this.engine, this.images});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // BACKGROUND
    final bgPaint = Paint()..shader = ui.Gradient.linear(const Offset(0, 0), const Offset(0, GameEngine.fieldSize), [const Color(0xFF050505), const Color(0xFF0A0A0A)]);
    canvas.drawRect(const Rect.fromLTWH(0, 0, GameEngine.fieldSize, GameEngine.fieldSize), bgPaint);
    
    // RADAR / GRID
    final radarLinePaint = Paint()..color = const Color(0xFFFFD700).withValues(alpha: 0.04)..style=PaintingStyle.stroke..strokeWidth=2;
    canvas.save();
    ...
    final sweepPaint = Paint()..shader = ui.Gradient.sweep(Offset.zero, [const Color(0xFFFFD700).withValues(alpha: 0.15), const Color(0xFFFFD700).withValues(alpha: 0.0)], [0.0, 0.2]);
    canvas.drawArc(Rect.fromCircle(center: Offset.zero, radius: 1000), 0, math.pi/2, true, sweepPaint);
    canvas.restore();

    
    // Sweeping Security Lasers on Floor
    final laserFloorBg = Paint()..color=Colors.redAccent.withValues(alpha: 0.3)..strokeWidth=4..maskFilter=const MaskFilter.blur(BlurStyle.normal, 3);
    final laserFloorFg = Paint()..color=Colors.white.withValues(alpha: 0.5)..strokeWidth=1;
    double laserY = (engine.time * 100) % GameEngine.fieldSize;
    canvas.drawLine(Offset(0, laserY), Offset(GameEngine.fieldSize, laserY), laserFloorBg);
    canvas.drawLine(Offset(0, laserY), Offset(GameEngine.fieldSize, laserY), laserFloorFg);
    ...
    final blockSidePaint = Paint()..color = const Color(0xFF000000);
    final blockLaserPaint = Paint()..color = Colors.amberAccent.withValues(alpha: 0.6)..strokeWidth = 2..maskFilter = const MaskFilter.blur(BlurStyle.normal, 3);
    final blockGlassPaint = Paint()..color = Colors.white.withValues(alpha: 0.2);
    final neonBorderPaint = Paint()..color = const Color(0xFFFFD700).withValues(alpha: 0.8)..style = PaintingStyle.stroke..strokeWidth = 2..maskFilter = const MaskFilter.blur(BlurStyle.solid, 3);
    ...
    for (int i = 0; i < engine.obstacles.length; i++) {
      ...
      // Top Face
      final topPaint = Paint()..shader = ui.Gradient.linear(obs.topCenter, obs.bottomCenter, [const Color(0xFF222222), const Color(0xFF111111)]);
    ...
    // TARGET PAINTS
    final targetPulsePaint = Paint()..color = const Color(0xFFFFD700).withValues(alpha: 0.4)..maskFilter = const MaskFilter.blur(BlurStyle.normal, 3);
    ...
    for (int i = 0; i < engine.targets.length; i++) {
      ...
      if (images != null && images!['target'] != null) {
        ...
        canvas.drawCircle(target + Offset(0, floatOffset), tr, Paint()..color = isNext ? Colors.amberAccent : Colors.grey);
      }
    }
    ...
    if (engine.exitGate != null) {
      ...
      canvas.drawCircle(Offset.zero, 40 * pulse, Paint()..color = const Color(0xFFFFD700).withValues(alpha: 0.5)..maskFilter = const MaskFilter.blur(BlurStyle.normal, 3));
      
      canvas.rotate(engine.time * 3);
      final pSweep = Paint()..shader = ui.Gradient.sweep(Offset.zero, [const Color(0xFFFFD700), Colors.white, const Color(0xFFFFD700)], [0.0, 0.5, 1.0]);
  ...
  @override bool shouldRepaint(covariant GamePainterA3Heist old) => true;
}

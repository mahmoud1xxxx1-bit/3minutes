...
class GamePainterA103 extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for(int i=0; i<40; i++) {
        ...
        if (p > 0) {
            canvas.drawCircle(Offset(lx, ly), 3 + p*2, Paint()..color = Colors.amber.withOpacity(0.6)..maskFilter = const MaskFilter.blur(BlurStyle.normal, 4));
    ...
    if (engine.exitGate != null) {
        canvas.drawCircle(engine.exitGate!, 45, Paint()..color=Colors.greenAccent.withOpacity(0.8)..style=PaintingStyle.stroke..strokeWidth=8);
        for(int i=0; i<8; i++) {

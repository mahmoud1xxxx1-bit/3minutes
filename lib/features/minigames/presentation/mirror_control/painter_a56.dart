...
class GamePainterA56 extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for(int i=0; i<40; i++) {
       ...
       canvas.drawCircle(Offset(mx, my), 10 + rnd.nextDouble()*20, Paint()..color=const Color(0xFF55AA55).withValues(alpha: 0.15)..maskFilter=const MaskFilter.blur(BlurStyle.normal, 3));
    }
    ...
    for(int i=0; i<8; i++) {
       ...
       // spots
       canvas.drawCircle(Offset(mx-10, my-5), 5, Paint()..color=Colors.cyanAccent.withValues(alpha: 0.7));
       canvas.drawCircle(Offset(mx+15, my-8), 4, Paint()..color=Colors.cyanAccent.withValues(alpha: 0.7));
    }
    ...
    for(int i = 0; i < 30; i++) {
      ...
      canvas.drawCircle(Offset(sx, sy), 3, Paint()..color=Colors.lightGreenAccent.withValues(alpha: 0.6)..maskFilter=const MaskFilter.blur(BlurStyle.solid, 3));
    }
    ...
    for (int i = 0; i < engine.targets.length; i++) {
        ...
        if (isActive) {
            ...
            canvas.drawCircle(target, 35*p, Paint()..color=Colors.limeAccent.withValues(alpha: 0.6)..maskFilter=const MaskFilter.blur(BlurStyle.normal, 3));
    ...
    if (engine.exitGate != null) {
        canvas.drawCircle(engine.exitGate!, 45, Paint()..color=Colors.pinkAccent.withValues(alpha: 0.3)..maskFilter=const MaskFilter.blur(BlurStyle.normal, 3));

...
class GamePainterA5WildWest extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // RADAR / GRID
    final radarLinePaint = Paint()..color = const Color(0xFFFF5500).withValues(alpha: 0.04)..style=PaintingStyle.stroke..strokeWidth=2;
    canvas.save();
    ...
    final sweepPaint = Paint()..shader = ui.Gradient.sweep(Offset.zero, [const Color(0xFFFF5500).withValues(alpha: 0.15), const Color(0xFFFF5500).withValues(alpha: 0.0)], [0.0, 0.2]);
    canvas.drawArc(Rect.fromCircle(center: Offset.zero, radius: 1000), 0, math.pi/2, true, sweepPaint);
    canvas.restore();

    
    // Heat Haze & Embers
    final emberPaint = Paint()..color = const Color(0xFFFF5500).withValues(alpha: 0.8);
    for (int i = 0; i < 35; i++) {
    ...
    final blockLaserPaint = Paint()..color = Colors.deepOrangeAccent.withValues(alpha: 0.6)..strokeWidth = 2..maskFilter = const MaskFilter.blur(BlurStyle.normal, 3);
    final blockGlassPaint = Paint()..color = Colors.white.withValues(alpha: 0.2);
    final neonBorderPaint = Paint()..color = const Color(0xFFFF5500).withValues(alpha: 0.8)..style = PaintingStyle.stroke..strokeWidth = 2..maskFilter = const MaskFilter.blur(BlurStyle.solid, 3);
    ...
    // TARGET PAINTS
    final targetPulsePaint = Paint()..color = const Color(0xFFFF5500).withValues(alpha: 0.4)..maskFilter = const MaskFilter.blur(BlurStyle.normal, 3);
    ...
    if (engine.exitGate != null) {
      ...
      canvas.drawCircle(Offset.zero, 40 * pulse, Paint()..color = const Color(0xFFFF5500).withValues(alpha: 0.5)..maskFilter = const MaskFilter.blur(BlurStyle.normal, 3));

...
class GamePainterA42 extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for(double x=0; x<GameEngine.fieldSize; x+=60) {
      canvas.drawLine(Offset(x, 0), Offset(x, GameEngine.fieldSize), plank);
    }
    ...
    for (int i = 0; i < engine.targets.length; i++) {
      ...
      if (isNext) {
          ...
          canvas.drawCircle(Offset.zero, tr*2, Paint()..shader = ui.Gradient.radial(Offset.zero, tr*2, [const Color(0xFFFFD54F).withValues(alpha: 0.4), Colors.transparent]));
      } else {
          // Empty stone

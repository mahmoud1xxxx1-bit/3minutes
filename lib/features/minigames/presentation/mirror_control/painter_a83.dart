...
class GamePainterA83 extends CustomPainter {
  GamePainterA83({required this.engine, this.images});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final bgGradient = RadialGradient(
      ...
      radius: 1.5,
      colors: [const Color(0xFF003049), const Color(0xFF000814)],
    ).createShader(Rect.fromLTWH(0, 0, GameEngine.fieldSize, GameEngine.fieldSize));
    canvas.drawRect(const Rect.fromLTWH(0, 0, GameEngine.fieldSize, GameEngine.fieldSize), Paint()..shader = bgGradient);
    
    // Glowing plankton
    for (int i = 0; i < 200; i++) {
      double px = rnd.nextDouble() * GameEngine.fieldSize;
      double py = rnd.nextDouble() * GameEngine.fieldSize;
      double pr = rnd.nextDouble() * 3 + 1;
      double opacity = 0.3 + 0.5 * math.sin(engine.time * (1 + rnd.nextDouble()));
      canvas.drawCircle(Offset(px, py), pr, Paint()..color = Colors.tealAccent.withValues(alpha: opacity)..maskFilter = const MaskFilter.blur(BlurStyle.normal, 2));
    }
    ...
    for (int i = 0; i < engine.obstacles.length; i++) {
        ...
        final rockPaint = Paint()..color = const Color(0xFF1E3A5F);
        canvas.drawRRect(RRect.fromRectAndRadius(obs, const Radius.circular(15)), rockPaint);
        
        // Algae patches
        final algaePaint = Paint()..color = Colors.teal.withValues(alpha: 0.6)..maskFilter = const MaskFilter.blur(BlurStyle.normal, 5);
        canvas.drawCircle(obs.topLeft + const Offset(10, 10), 15, algaePaint);
        canvas.drawCircle(obs.bottomRight - const Offset(10, 10), 20, algaePaint);
    }
    ...
    for (int i = 0; i < engine.targets.length; i++) {
        ...
        // Anglerfish glowing bulb
        canvas.drawCircle(target, 12 + 2 * math.sin(engine.time * 4), Paint()..color = Colors.tealAccent);
        canvas.drawCircle(target, 8, Paint()..color = Colors.white);
  ...
  @override bool shouldRepaint(covariant GamePainterA83 old) => true;
}

...
class GamePainterA63 extends CustomPainter {
  GamePainterA63({required this.engine, this.images});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final rnd = math.Random(63);

    // 1. FLOOR & BACKGROUND (Crystal Caves)
    canvas.drawRect(const Rect.fromLTWH(0, 0, GameEngine.fieldSize, GameEngine.fieldSize), Paint()..color = const Color(0xFF0A051A));
    ...
    for (int i = 0; i < engine.obstacles.length; i++) {
        ...
        canvas.drawRect(obs.shift(const Offset(0, 15)), Paint()..color=Colors.black87..maskFilter=const MaskFilter.blur(BlurStyle.normal, 5));
        canvas.drawRect(obs, Paint()..color=Colors.purple[900]!);
        canvas.drawRect(obs.deflate(4), Paint()..color=Colors.purple[400]!..style=PaintingStyle.stroke..strokeWidth=2);
    ...
    for (int i = 0; i < engine.targets.length; i++) {
        ...
        gem.lineTo(target.dx + 12, target.dy);
        gem.lineTo(target.dx, target.dy + 15);
        gem.lineTo(target.dx - 12, target.dy);
        gem.close();
        canvas.drawPath(gem, Paint()..color=Colors.cyanAccent);
    ...
    if (engine.exitGate != null) {
        canvas.drawCircle(engine.exitGate!, 45, Paint()..color=Colors.purpleAccent..style=PaintingStyle.stroke..strokeWidth=8..maskFilter=const MaskFilter.blur(BlurStyle.normal, 4));
  ...
  @override bool shouldRepaint(covariant GamePainterA63 old) => true;
}

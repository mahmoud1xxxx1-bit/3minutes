...
class GamePainterA49 extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final tile = Paint()..color=Colors.white.withValues(alpha: 0.9);
    final shadow = Paint()..color=const Color(0xFF0277BD).withValues(alpha: 0.3);
    ...
    for (int i = 0; i < 8; i++) {
      ...
      canvas.drawCircle(Offset(px, py), 40, Paint()..color=Colors.white.withValues(alpha: 0.5));
      canvas.drawCircle(Offset(px+30, py+10), 30, Paint()..color=Colors.white.withValues(alpha: 0.5));
      canvas.drawCircle(Offset(px-30, py+10), 30, Paint()..color=Colors.white.withValues(alpha: 0.5));
    }
    ...
    for (int i = 0; i < engine.targets.length; i++) {
      ...
      if (isNext) {
          ...
          canvas.restore();
          canvas.drawCircle(Offset.zero, tr*2.5, Paint()..shader = ui.Gradient.radial(Offset.zero, tr*2.5, [const Color(0xFFFFD700).withValues(alpha: 0.5), Colors.transparent]));
      } else {
          // Extinguished orb

...
class GamePainterA43 extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < 20; i++) {
      ...
      canvas.drawCircle(Offset(px, py), 6, Paint()..color=Colors.white.withOpacity(0.6));
    }
    ...
    for (int i = 0; i < engine.obstacles.length; i++) {
      ...
      canvas.drawRect(obs.shift(const Offset(-5, -5)).deflate(10.0), Paint()..color=const Color(0xFFEF5350).withOpacity(0.5)); // highlight
    }
    ...
    for (int i = 0; i < engine.targets.length; i++) {
      ...
      if (isNext) {
          ...
          for(double y=-15; y<=15; y+=40) {
              ...
              canvas.drawLine(Offset(x, y), Offset(-x, y), Paint()..color=Colors.white.withOpacity(0.5)..strokeWidth=1);
          }
          canvas.restore();
          canvas.drawCircle(Offset.zero, tr*2, Paint()..shader = ui.Gradient.radial(Offset.zero, tr*2, [const Color(0xFF00E5FF).withOpacity(0.4), Colors.transparent]));
      } else {
          // Dead protein

...
class GamePainterA37 extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for(double y=0; y<GameEngine.fieldSize; y+=40) {
        ...
        for(double x=0; x<=GameEngine.fieldSize; x+=50) {
            if(x==0) {
              ...
              w.quadraticBezierTo(x-25, y + math.sin(x)*10, x, y);
            }
        }
    ...
    canvas.drawRect(const Rect.fromLTWH(0, 0, 400, 300), Paint()..color=const Color(0xFF1976D2).withValues(alpha: 0.8));
    canvas.drawCircle(const Offset(200, 150), 100, Paint()..color=Colors.white.withValues(alpha: 0.5)..style=PaintingStyle.stroke..strokeWidth=2);
    canvas.restore();
    ...
    for (int i = 0; i < 30; i++) {
      ...
      canvas.drawCircle(Offset(px, py), 2, Paint()..color=Colors.white.withValues(alpha: 0.4)); 
    }
    ...
    for (int i = 0; i < engine.targets.length; i++) {
      ...
      if (isNext) {
          ...
          canvas.drawCircle(Offset.zero, tr*2, Paint()..shader = ui.Gradient.radial(Offset.zero, tr*2, [const Color(0xFFFF1744).withValues(alpha: 0.5), Colors.transparent]));
      } else {
          // Tiny brass gear

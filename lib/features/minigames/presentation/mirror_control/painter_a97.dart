...
class GamePainterA97 extends CustomPainter {
  GamePainterA97({required this.engine, this.images});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // ==========================================
    canvas.drawRect(const Rect.fromLTWH(0, 0, GameEngine.fieldSize, GameEngine.fieldSize), Paint()..color = const Color(0xff110000));
    
    for (int i=0; i<40; i++) {
        double x = rnd.nextDouble() * GameEngine.fieldSize;
        double y = rnd.nextDouble() * GameEngine.fieldSize;
        double r = 20 + rnd.nextDouble() * 30;
        canvas.drawCircle(Offset(x, y), r, Paint()..color=Colors.red.withValues(alpha: 0.1)..maskFilter=const MaskFilter.blur(BlurStyle.normal, 3));
    }
    ...
    for (int i = 0; i < engine.obstacles.length; i++) {
        ...
        canvas.drawRect(obs.shift(const Offset(0, 15)), Paint()..color=Colors.black); 
        canvas.drawRect(obs, Paint()..color=const Color(0xff222222)); 
        // obsidian spikes
        canvas.drawCircle(obs.center, 5, Paint()..color=Colors.redAccent);
    }
    ...
    for (int i = 0; i < engine.targets.length; i++) {
        ...
        canvas.drawCircle(target, 15, Paint()..color=Colors.deepOrange);
        canvas.drawCircle(target, 8, Paint()..color=Colors.yellow);
  ...
  @override bool shouldRepaint(covariant GamePainterA97 old) => true;
}

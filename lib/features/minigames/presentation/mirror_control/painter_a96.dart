...
class GamePainterA96 extends CustomPainter {
  GamePainterA96({required this.engine, this.images});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final rnd = math.Random(96);

    // ==========================================
    // 1. ARTISTIC BACKGROUND AND FLOOR
    // ==========================================
    canvas.drawRect(const Rect.fromLTWH(0, 0, GameEngine.fieldSize, GameEngine.fieldSize), Paint()..color = const Color(0xfffdf6e3));
    
    for (int i=0; i<30; i++) {
        double x = rnd.nextDouble() * GameEngine.fieldSize;
        double y = rnd.nextDouble() * GameEngine.fieldSize;
        double r = 40 + rnd.nextDouble() * 60;
        Path paper = Path()
            ..moveTo(x, y)
            ..lineTo(x + r, y + r*0.3)
            ..lineTo(x + r*0.6, y + r*1.2)
            ..close();
        canvas.drawPath(paper, Paint()..color = Colors.accents[rnd.nextInt(Colors.accents.length)].withValues(alpha: 0.2));
        canvas.drawPath(paper, Paint()..color = Colors.black12..style=PaintingStyle.stroke);
    }
    ...
    for (int i = 0; i < engine.obstacles.length; i++) {
        ...
        canvas.drawRect(obs.shift(const Offset(0, 10)), Paint()..color=Colors.black26); // Shadow
        canvas.drawRect(obs, Paint()..color=Colors.white); // Body
        Path fold = Path()
            ..moveTo(obs.left, obs.top)
            ..lineTo(obs.right, obs.bottom)
            ..moveTo(obs.right, obs.top)
            ..lineTo(obs.left, obs.bottom);
        canvas.drawPath(fold, Paint()..color=Colors.black12..style=PaintingStyle.stroke);
    }
    ...
    for (int i = 0; i < engine.targets.length; i++) {
        ...
        Path lotus = Path()
            ..moveTo(target.dx, target.dy - 15)
            ..lineTo(target.dx + 15, target.dy)
            ..lineTo(target.dx, target.dy + 15)
            ..lineTo(target.dx - 15, target.dy)
            ..close();
        canvas.drawPath(lotus, Paint()..color=Colors.pinkAccent);
        canvas.drawPath(lotus, Paint()..color=Colors.black..style=PaintingStyle.stroke);
  ...
  @override bool shouldRepaint(covariant GamePainterA96 old) => true;
}

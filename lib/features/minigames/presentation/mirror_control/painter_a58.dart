...
class GamePainterA58 extends CustomPainter {
  GamePainterA58({required this.engine, this.images});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawRect(
      ...
      Paint()..shader = ui.Gradient.radial(
        ...
        [const Color(0xFF0A1B2E), const Color(0xFF030811), const Color(0xFF000000)],
        [0.0, 0.5, 1.0]
      )
    );
    for(int i = 0; i < 15; i++) {
        canvas.drawCircle(Offset(rnd.nextDouble() * GameEngine.fieldSize, rnd.nextDouble() * GameEngine.fieldSize), rnd.nextDouble() * 100 + 50, Paint()..color = const Color(0xFF14304D).withValues(alpha: 0.3)..maskFilter=const MaskFilter.blur(BlurStyle.normal, 3));
    }
    for (int i = 0; i < engine.obstacles.length; i++) {
        final obs = engine.obstacles[i];
        canvas.drawRect(obs.shift(const Offset(0, 15)), Paint()..color=Colors.black.withValues(alpha: 0.8));
        canvas.drawRect(obs, Paint()..color=const Color(0xFF285C5E));
        canvas.drawRect(obs, Paint()..color=const Color(0xFF4FA9A5)..style=PaintingStyle.stroke..strokeWidth=2);
    }
    for (int i = 0; i < engine.targets.length; i++) {
        ...
        if (isActive) {
            ...
            canvas.drawCircle(target, 35*p, Paint()..color=Colors.cyanAccent.withValues(alpha: 0.6)..maskFilter=const MaskFilter.blur(BlurStyle.normal, 3));
    ...
    if (engine.exitGate != null) {
        canvas.drawCircle(engine.exitGate!, 45, Paint()..color=Colors.indigoAccent..style=PaintingStyle.stroke..strokeWidth=8);
    }
  ...
  @override bool shouldRepaint(covariant GamePainterA58 old) => true;
}

...
class GamePainterA80 extends CustomPainter {
  GamePainterA80({required this.engine, this.images});
...
// deleted
    ...
    for (double i = 0; i < GameEngine.fieldSize; i+=80) {
      for (double j = 0; j < GameEngine.fieldSize; j+=80) {
        canvas.drawRect(Rect.fromLTWH(i, j, 78, 78), Paint()..color = Colors.grey.shade900);
      }
    }
    // Red velvet carpet
    canvas.drawRect(Rect.fromLTWH(GameEngine.fieldSize/2 - 100, 0, 200, GameEngine.fieldSize), Paint()..color=Colors.red.shade900);
    canvas.drawRect(Rect.fromLTWH(GameEngine.fieldSize/2 - 100, 0, 10, GameEngine.fieldSize), Paint()..color=Colors.yellow.shade700);
    canvas.drawRect(Rect.fromLTWH(GameEngine.fieldSize/2 + 90, 0, 10, GameEngine.fieldSize), Paint()..color=Colors.yellow.shade700);
    ...
    for (int i = 0; i < engine.obstacles.length; i++) {
        ...
        canvas.drawRect(obs.shift(const Offset(0, 15)), Paint()..color=Colors.black54); // Shadow
        // Black coffins
        canvas.drawPath(Path()..moveTo(obs.left + obs.width/2, obs.top)
            ..lineTo(obs.right, obs.top + 20)..lineTo(obs.right - 10, obs.bottom)
            ..lineTo(obs.left + 10, obs.bottom)..lineTo(obs.left, obs.top + 20)..close(), 
            Paint()..color=Colors.black);
        canvas.drawPath(Path()..moveTo(obs.left + obs.width/2, obs.top)
            ..lineTo(obs.right, obs.top + 20)..lineTo(obs.right - 10, obs.bottom)
            ..lineTo(obs.left + 10, obs.bottom)..lineTo(obs.left, obs.top + 20)..close(), 
            Paint()..color=Colors.red.shade900..style=PaintingStyle.stroke..strokeWidth=2);
        // Candles
        canvas.drawRect(Rect.fromLTWH(obs.left-10, obs.top-10, 6, 15), Paint()..color=Colors.white);
        canvas.drawCircle(Offset(obs.left-7, obs.top-12), 4, Paint()..color=Colors.orangeAccent);
    }
    ...
    for (int i = 0; i < engine.targets.length; i++) {
        ...
        // Blood vial target
        canvas.drawRect(Rect.fromCenter(center: target, width: 12, height: 25), Paint()..color=Colors.grey.shade300);
        canvas.drawRect(Rect.fromCenter(center: target + const Offset(0, 5), width: 10, height: 15), Paint()..color=Colors.red);
  ...
  @override bool shouldRepaint(covariant GamePainterA80 old) => true;
}

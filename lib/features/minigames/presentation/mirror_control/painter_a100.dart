...
class GamePainterA100 extends CustomPainter {
  GamePainterA100({required this.engine, this.images});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final rnd = math.Random(100);

    // ==========================================
    // 1. ARTISTIC BACKGROUND AND FLOOR
    // ==========================================
    canvas.drawRect(const Rect.fromLTWH(0, 0, GameEngine.fieldSize, GameEngine.fieldSize), Paint()..color = const Color(0xff2d1b2e));
    
    // Scattered popcorn
    for (int i=0; i<80; i++) {
        double x = rnd.nextDouble() * GameEngine.fieldSize;
        double y = rnd.nextDouble() * GameEngine.fieldSize;
        canvas.drawCircle(Offset(x, y), 3, Paint()..color=Colors.yellow.shade100);
    }
    
    // Dim carnival lighting
    canvas.drawRect(const Rect.fromLTWH(0, 0, GameEngine.fieldSize, GameEngine.fieldSize), Paint()..color = Colors.black54);
    ...
    for (int i = 0; i < engine.obstacles.length; i++) {
        ...
        canvas.drawRect(obs, Paint()..color=const Color(0xff6a2c3a)); 
        // Faded stripes
        for (double x = obs.left + 5; x < obs.right; x+=60) {
            canvas.drawRect(Rect.fromLTWH(x, obs.top, 5, obs.height), Paint()..color=Colors.white24);
        }
    }
    ...
    for (int i = 0; i < engine.targets.length; i++) {
        ...
        canvas.drawCircle(target, 15, Paint()..color=Colors.purple);
        canvas.drawCircle(target, 8, Paint()..color=Colors.orange);
  ...
  @override bool shouldRepaint(covariant GamePainterA100 old) => true;
}

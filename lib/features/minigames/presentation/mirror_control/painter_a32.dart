...
class GamePainterA32 extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < 40; i++) {
      ...
      canvas.drawCircle(Offset(px, py), 8, Paint()..shader=ui.Gradient.radial(Offset(px, py), 8, [const Color(0xFF00E5FF).withValues(alpha: 0.5), Colors.transparent]));
    }
    ...
    for (int i = 0; i < engine.targets.length; i++) {
      ...
      if (isNext) {
          ...
          canvas.drawCircle(Offset.zero, tr*2, Paint()..shader = ui.Gradient.radial(Offset.zero, tr*2, [const Color(0xFF00E5FF).withValues(alpha: 0.6), Colors.transparent]));
      } else {
          // Stone lantern unlit

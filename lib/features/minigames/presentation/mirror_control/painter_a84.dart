...
class GamePainterA84 extends CustomPainter {
  GamePainterA84({required this.engine, this.images});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final rnd = math.Random(84);

    // ==========================================
    // 1. ARTISTIC BACKGROUND AND FLOOR
    // ==========================================
    canvas.drawRect(const Rect.fromLTWH(0, 0, GameEngine.fieldSize, GameEngine.fieldSize), Paint()..color = const Color(0xFFE3F2FD));
    
    final cloudPaint = Paint()..color = Colors.white.withValues(alpha: 0.7)..maskFilter = const MaskFilter.blur(BlurStyle.normal, 3);
    for (int i = 0; i < 20; i++) {
      double cx = rnd.nextDouble() * GameEngine.fieldSize;
      double cy = rnd.nextDouble() * GameEngine.fieldSize;
      canvas.drawCircle(Offset(cx, cy), 60 + rnd.nextDouble() * 40, cloudPaint);
      canvas.drawCircle(Offset(cx + 40, cy + 20), 50 + rnd.nextDouble() * 30, cloudPaint);
      canvas.drawCircle(Offset(cx - 30, cy + 10), 70 + rnd.nextDouble() * 20, cloudPaint);
    }
    ...
    for (int i = 0; i < engine.obstacles.length; i++) {
        ...
        canvas.drawRect(obs.shift(const Offset(0, 20)), Paint()..color=Colors.black26); // Soft shadow
        
        // Marble pillar look
        canvas.drawRect(obs, Paint()..color = Colors.white);
        
        final detailsPaint = Paint()..color = Colors.grey[300]!..style = PaintingStyle.stroke..strokeWidth = 2;
        for (double x = obs.left + 5; x < obs.right; x+=60) {
           canvas.drawLine(Offset(x, obs.top), Offset(x, obs.bottom), detailsPaint);
        }
        
        // Golden accents
        canvas.drawRect(Rect.fromLTWH(obs.left, obs.top, obs.width, 10), Paint()..color = Colors.amber);
        canvas.drawRect(Rect.fromLTWH(obs.left, obs.bottom - 10, obs.width, 10), Paint()..color = Colors.amber);
    }
    ...
    for (int i = 0; i < engine.targets.length; i++) {
        ...
        canvas.drawCircle(target, 16, Paint()..color = Colors.amber..style = PaintingStyle.stroke..strokeWidth = 3);
        final leafPaint = Paint()..color = Colors.amberAccent;
        for (double a = 0; a < math.pi * 2; a += math.pi / 4) {
          canvas.drawCircle(target + Offset(math.cos(a) * 16, math.sin(a) * 16), 4, leafPaint);
        }
  ...
  @override bool shouldRepaint(covariant GamePainterA84 old) => true;
}

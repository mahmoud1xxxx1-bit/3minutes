...
class GamePainterA92 extends CustomPainter {
  GamePainterA92({required this.engine, this.images});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final rnd = math.Random(92);

    // ==========================================
    // 1. ARTISTIC BACKGROUND AND FLOOR
    // ==========================================
    canvas.drawRect(const Rect.fromLTWH(0, 0, GameEngine.fieldSize, GameEngine.fieldSize), Paint()..color = const Color(0xFF0A1F0F));
    for(int i=0; i<40; i++) {
        double x1 = rnd.nextDouble() * GameEngine.fieldSize;
        double y1 = rnd.nextDouble() * GameEngine.fieldSize;
        double l1 = 20 + rnd.nextDouble() * 100;
        bool isHoriz = rnd.nextBool();
        Offset p1 = Offset(x1, y1);
        Offset p2 = isHoriz ? Offset(x1 + l1, y1) : Offset(x1, y1 + l1);
        canvas.drawLine(p1, p2, Paint()..color=const Color(0xFF39FF14).withValues(alpha: 0.3)..strokeWidth=3);
        canvas.drawCircle(p1, 4, Paint()..color=const Color(0xFFFFD700).withValues(alpha: 0.5));
    }
    ...
    for (int i = 0; i < engine.obstacles.length; i++) {
        ...
        canvas.drawRect(obs, Paint()..color=const Color(0xFF2B2B2B));
        canvas.drawRect(obs.deflate(4), Paint()..color=const Color(0xFF1E1E1E));
        
        for(double x = obs.left + 5; x < obs.right; x+=60) {
            canvas.drawRect(Rect.fromLTWH(x, obs.top - 2, 8, 4), Paint()..color=const Color(0xFFC0C0C0));
            canvas.drawRect(Rect.fromLTWH(x, obs.bottom - 2, 8, 4), Paint()..color=const Color(0xFFC0C0C0));
        }
    }
    ...
    for (int i = 0; i < engine.targets.length; i++) {
        ...
        // Capacitor
        canvas.drawRect(Rect.fromCenter(center: target, width: 24, height: 24), Paint()..color=const Color(0xFF39FF14));
        canvas.drawRect(Rect.fromCenter(center: target, width: 16, height: 16), Paint()..color=const Color(0xFF222222));
  ...
  @override bool shouldRepaint(covariant GamePainterA92 old) => true;
}

...
class GamePainterA54 extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for(double i=0; i<GameEngine.fieldSize; i+=50) {
       canvas.drawLine(Offset(i, 0), Offset(i, GameEngine.fieldSize), Paint()..color=const Color(0xFF26C485).withValues(alpha: 0.2)..strokeWidth=2);
       canvas.drawLine(Offset(0, i), Offset(GameEngine.fieldSize, i), Paint()..color=const Color(0xFF26C485).withValues(alpha: 0.2)..strokeWidth=2);
    }
    ...
    for(int i=0; i<5; i++) {
       ...
       canvas.drawCircle(Offset(nx, ny), 40, Paint()..color=const Color(0xFFFF007F).withValues(alpha: 0.1)..maskFilter=const MaskFilter.blur(BlurStyle.normal, 3));
    ...
    for (int i = 0; i < engine.obstacles.length; i++) {
        ...
        canvas.drawRect(obs.shift(const Offset(0, 15)), Paint()..color=const Color(0xFF000000).withValues(alpha: 0.8));
    ...
    if (engine.exitGate != null) {
        canvas.drawRect(Rect.fromCenter(center: engine.exitGate!, width: 70, height: 70), Paint()..color=Colors.redAccent.withValues(alpha: 0.4)..maskFilter=const MaskFilter.blur(BlurStyle.normal, 3));

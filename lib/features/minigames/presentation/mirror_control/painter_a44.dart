...
class GamePainterA44 extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawRect(const Rect.fromLTWH(200, 200, 600, 600), Paint()..shader = ui.Gradient.sweep(const Offset(500, 500), [Colors.red.withValues(alpha: 0.2), Colors.blue.withValues(alpha: 0.2), Colors.green.withValues(alpha: 0.2), Colors.yellow.withValues(alpha: 0.2), Colors.red.withValues(alpha: 0.2)], [0.0, 0.25, 0.5, 0.75, 1.0]));
    ...
    for (int i = 0; i < 15; i++) {
      ...
      canvas.drawCircle(Offset(px, py), 2, Paint()..color=Colors.white.withValues(alpha: 0.3)); // holy motes
    }
    ...
    for (int i = 0; i < engine.targets.length; i++) {
      ...
      if (isNext) {
          ...
          canvas.drawCircle(Offset.zero, tr*2, Paint()..shader = ui.Gradient.radial(Offset.zero, tr*2, [const Color(0xFFFFD700).withValues(alpha: 0.5), Colors.transparent]));
          // Light beam from above
          canvas.drawRect(const Rect.fromLTWH(-10, -100, 20, 100), Paint()..shader=ui.Gradient.linear(const Offset(0, -100), const Offset(0, 0), [Colors.white.withValues(alpha: 0.0), Colors.white.withValues(alpha: 0.4)]));
      } else {
          // Empty stone bowl
    ...
    if (engine.exitGate != null) {
      ...
      for(int a=0; a<8; a++) {
          ...
          canvas.drawPath(petal, Paint()..color=[Colors.red, Colors.blue][a%2].withValues(alpha: 0.8));

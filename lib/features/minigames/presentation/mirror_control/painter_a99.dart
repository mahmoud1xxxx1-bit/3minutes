...
class GamePainterA99 extends CustomPainter {
  GamePainterA99({required this.engine, this.images});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // ==========================================
    canvas.drawRect(const Rect.fromLTWH(0, 0, GameEngine.fieldSize, GameEngine.fieldSize), Paint()..color = const Color(0xff0a4b66));
    
    for (int i=0; i<50; i++) {
        double x = rnd.nextDouble() * GameEngine.fieldSize;
        double y = rnd.nextDouble() * GameEngine.fieldSize;
        double s = rnd.nextDouble() * 20;
        canvas.drawCircle(Offset(x, y), s, Paint()..color=Colors.tealAccent.withValues(alpha: 0.1)..maskFilter=const MaskFilter.blur(BlurStyle.normal, 3));
    }
    ...
    for (int i = 0; i < engine.obstacles.length; i++) {
        ...
        canvas.drawRect(obs, Paint()..color=const Color(0xffb0c4de)); 
        // Greek pillars texture
        for (double x = obs.left + 10; x < obs.right; x+=60) {
            canvas.drawLine(Offset(x, obs.top), Offset(x, obs.bottom), Paint()..color=Colors.white30..strokeWidth=2);
        }
    }
    ...
    for (int i = 0; i < engine.targets.length; i++) {
        ...
        canvas.drawCircle(target, 15, Paint()..color=Colors.tealAccent);
        canvas.drawCircle(target, 8, Paint()..color=Colors.lightBlueAccent);
  ...
  @override bool shouldRepaint(covariant GamePainterA99 old) => true;
}

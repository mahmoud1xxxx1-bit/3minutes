...
class GamePainterA55 extends CustomPainter {
  ...
  void drawGear(Canvas canvas, Offset center, double radius, double angle, Color color) {
    ...
    for(int i=0; i<teeth; i++) {
      ...
      if(i==0) {
        gearPath.moveTo(math.cos(a1)*radius, math.sin(a1)*radius);
      } else {
        gearPath.lineTo(math.cos(a1)*radius, math.sin(a1)*radius);
      }
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for(int i = 0; i < 20; i++) {
      ...
      canvas.drawCircle(Offset(sx, sy), 15 + rnd.nextDouble()*20, Paint()..color=Colors.white.withValues(alpha: 0.1)..maskFilter=const MaskFilter.blur(BlurStyle.normal, 3));
    }
    ...
    if (engine.exitGate != null) {
        canvas.drawCircle(engine.exitGate!, 45, Paint()..color=Colors.amber.withValues(alpha: 0.3)..maskFilter=const MaskFilter.blur(BlurStyle.normal, 3));

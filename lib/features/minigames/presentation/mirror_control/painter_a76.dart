...
class GamePainterA76 extends CustomPainter {
  GamePainterA76({required this.engine, this.images});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawRect(
        ...
        Paint()..shader = ui.Gradient.linear(
            const Offset(0, 0), const Offset(GameEngine.fieldSize, GameEngine.fieldSize),
            [Colors.deepPurple.shade900, Colors.black]
        ));
    final glassPaint = Paint()..color = Colors.purple.shade800.withOpacity(0.5)..style = PaintingStyle.stroke..strokeWidth = 3;
    for(double x=0; x<GameEngine.fieldSize; x+=100) {
      for(double y=0; y<GameEngine.fieldSize; y+=100) {
        canvas.drawCircle(Offset(x+50, y+50), 40, glassPaint);
        canvas.drawCircle(Offset(x+50, y+50), 30, Paint()..color=Colors.purpleAccent.withOpacity(0.2));
      }
    }
    ...
    for (int i = 0; i < engine.obstacles.length; i++) {
        ...
        canvas.drawRect(obs, Paint()..color=Colors.grey.shade800);
        canvas.drawPath(Path()
            ..moveTo(obs.left, obs.top)
            ..lineTo(obs.left + obs.width / 2, obs.top - 20)
            ..lineTo(obs.right, obs.top)
            ..close(), Paint()..color=Colors.grey.shade900);
    }
    ...
    for (int i = 0; i < engine.targets.length; i++) {
        ...
        canvas.drawRect(Rect.fromCenter(center: target + const Offset(0, 5), width: 10, height: 10), Paint()..color=Colors.amber.shade700);
  ...
  @override bool shouldRepaint(covariant GamePainterA76 old) => true;
}

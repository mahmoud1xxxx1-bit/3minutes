...
class GamePainterA95 extends CustomPainter {
  GamePainterA95({required this.engine, this.images});
...
// deleted
    ...
    // ==========================================
    canvas.drawRect(const Rect.fromLTWH(0, 0, GameEngine.fieldSize, GameEngine.fieldSize), Paint()..color = const Color(0xFF1E1032));
    
    canvas.save();
    canvas.translate(GameEngine.fieldSize / 2, GameEngine.fieldSize / 2);
    canvas.rotate(engine.time * 0.1);
    Paint circlePaint = Paint()..color=const Color(0xFF9370DB).withValues(alpha: 0.3)..style=PaintingStyle.stroke..strokeWidth=2;
    canvas.drawCircle(Offset.zero, GameEngine.fieldSize*0.4, circlePaint);
    canvas.drawCircle(Offset.zero, GameEngine.fieldSize*0.35, circlePaint);
    
    Path star = Path();
    double r = GameEngine.fieldSize*0.35;
    for(int i=0; i<6; i++) {
        double angle = i * math.pi / 3;
        if (i==0) {
          star.moveTo(math.cos(angle)*r, math.sin(angle)*r);
        } else {
          star.lineTo(math.cos(angle)*r, math.sin(angle)*r);
        }
    }
    star.close();
    canvas.drawPath(star, circlePaint);
    canvas.restore();
    ...
    for (int i = 0; i < engine.obstacles.length; i++) {
        ...
        canvas.drawCircle(Offset(obs.left + 15, obs.top + 15), 8, Paint()..color=const Color(0xFF32CD32).withValues(alpha: 0.8));
        canvas.drawCircle(Offset(obs.left + 15, obs.top + 15), 4, Paint()..color=Colors.white.withValues(alpha: 0.5));
    }
    ...
    for (int i = 0; i < engine.targets.length; i++) {
        ...
        Path flask = Path()
           ..moveTo(target.dx - 4, target.dy - 12)
           ..lineTo(target.dx + 4, target.dy - 12)
           ..lineTo(target.dx + 4, target.dy - 4)
           ..lineTo(target.dx + 12, target.dy + 8)
           ..lineTo(target.dx - 12, target.dy + 8)
           ..lineTo(target.dx - 4, target.dy - 4)
           ..close();
        canvas.drawPath(flask, Paint()..color=const Color(0xFF00FFFF));
        canvas.drawCircle(target, 2, Paint()..color=Colors.white);
  ...
  @override bool shouldRepaint(covariant GamePainterA95 old) => true;
}

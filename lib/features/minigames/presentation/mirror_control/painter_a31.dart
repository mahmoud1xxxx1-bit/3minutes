...
class GamePainterA31 extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < 40; i++) {
      ...
      canvas.drawCircle(Offset(px, py), 15 + (i%10).toDouble(), Paint()..color=Colors.white.withValues(alpha: 0.05)); // thick steam
    }
    ...
    for (int i = 0; i < engine.targets.length; i++) {
      ...
      if (isNext) {
          ...
          canvas.drawCircle(Offset.zero, tr*2, Paint()..shader = ui.Gradient.radial(Offset.zero, tr*2, [const Color(0xFFFF9800).withValues(alpha: 0.6), Colors.transparent]));

...
class GamePainterA86 extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawPath(lavaPath, Paint()..color = Colors.orangeAccent.withValues(alpha: 0.8)..maskFilter = const MaskFilter.blur(BlurStyle.normal, 3));
    ...
    for (int i=0; i<100; i++) {
        ...
        canvas.drawCircle(Offset(rX, rY), rnd.nextDouble() * 2 + 1, Paint()..color = Colors.grey.withValues(alpha: 0.5));
    }
    ...
    for (int i = 0; i < engine.obstacles.length; i++) {
        ...
        canvas.drawRect(obs.shift(const Offset(0, 10)), Paint()..color=Colors.black.withValues(alpha: 0.8)); // Shadow
    ...
    if (engine.exitGate != null) {
        ...
        canvas.drawCircle(engine.exitGate!, 50, Paint()..color=const Color(0xFFFF2200).withValues(alpha: 0.5)..style=PaintingStyle.stroke..strokeWidth=15..maskFilter=const MaskFilter.blur(BlurStyle.normal, 3));
    }

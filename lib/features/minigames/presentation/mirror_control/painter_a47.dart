...
class GamePainterA47 extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final line = Paint()..color=const Color(0xFF00FFFF).withOpacity(0.2)..strokeWidth=1;
    final rnd = math.Random(47);
    List<Offset> nodes = [];
    for(int i=0; i<30; i++) nodes.add(Offset(rnd.nextDouble()*GameEngine.fieldSize, rnd.nextDouble()*GameEngine.fieldSize));
    ...
    for (int i = 0; i < engine.targets.length; i++) {
      ...
      if (isNext) {
          ...
          canvas.restore();
          canvas.drawCircle(Offset.zero, tr*2, Paint()..shader = ui.Gradient.radial(Offset.zero, tr*2, [const Color(0xFFFF00FF).withOpacity(0.6), Colors.transparent]));
      } else {
    ...
    if (engine.exitGate != null) {
      ...
      for(double r=10; r<=40; r+=40) {
          canvas.drawCircle(Offset.zero, r, Paint()..color=const Color(0xFF00FFFF).withOpacity(0.5)..style=PaintingStyle.stroke..strokeWidth=2);
      }

...
class GamePainterA1Forest extends CustomPainter {
  GamePainterA1Forest({required this.engine, this.images});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // BACKGROUND
    final bgPaint = Paint()..shader = ui.Gradient.linear(const Offset(0, 0), const Offset(0, GameEngine.fieldSize), [const Color(0xFF000803), const Color(0xFF001A0A)]);
    canvas.drawRect(const Rect.fromLTWH(0, 0, GameEngine.fieldSize, GameEngine.fieldSize), bgPaint);
    
    // RADAR / GRID
    final radarLinePaint = Paint()..color = const Color(0xFF00FF88).withValues(alpha: 0.04)..style=PaintingStyle.stroke..strokeWidth=2;
    canvas.save();
    ...
    final sweepPaint = Paint()..shader = ui.Gradient.sweep(Offset.zero, [const Color(0xFF00FF88).withValues(alpha: 0.15), const Color(0xFF00FF88).withValues(alpha: 0.0)], [0.0, 0.2]);
    canvas.drawArc(Rect.fromCircle(center: Offset.zero, radius: 1000), 0, math.pi/2, true, sweepPaint);
    canvas.restore();

    
    // Falling Neon Leaves
    final leafPaint = Paint()..color = const Color(0xFF00FF88).withValues(alpha: 0.6);
    for (int i = 0; i < 40; i++) {
      double px = ((i * 87.53) + math.sin(engine.time + i)*20) % GameEngine.fieldSize;
      double speed = 10 + (i % 5) * 5;
      double py = (engine.time * speed + i * 133.3) % GameEngine.fieldSize;
      canvas.drawOval(Rect.fromCenter(center: Offset(px, py), width: 6, height: 3), leafPaint);
    }
    ...
    final blockSidePaint = Paint()..color = const Color(0xFF000A03);
    final blockLaserPaint = Paint()..color = Colors.greenAccent.withValues(alpha: 0.6)..strokeWidth = 2..maskFilter = const MaskFilter.blur(BlurStyle.normal, 3);
    final blockGlassPaint = Paint()..color = Colors.white.withValues(alpha: 0.2);
    final neonBorderPaint = Paint()..color = const Color(0xFF00FF88).withValues(alpha: 0.8)..style = PaintingStyle.stroke..strokeWidth = 2..maskFilter = const MaskFilter.blur(BlurStyle.solid, 3);
    ...
    for (int i = 0; i < engine.obstacles.length; i++) {
      ...
      // Top Face
      final topPaint = Paint()..shader = ui.Gradient.linear(obs.topCenter, obs.bottomCenter, [const Color(0xFF00220A), const Color(0xFF001105)]);
    ...
    // TARGET PAINTS
    final targetPulsePaint = Paint()..color = const Color(0xFF00FF88).withValues(alpha: 0.4)..maskFilter = const MaskFilter.blur(BlurStyle.normal, 3);
    ...
    for (int i = 0; i < engine.targets.length; i++) {
      ...
      if (images != null && images!['target'] != null) {
        ...
        canvas.drawCircle(target + Offset(0, floatOffset), tr, Paint()..color = isNext ? Colors.greenAccent : Colors.grey);
      }
    }
    ...
    if (engine.exitGate != null) {
      ...
      canvas.drawCircle(Offset.zero, 40 * pulse, Paint()..color = const Color(0xFF00FF88).withValues(alpha: 0.5)..maskFilter = const MaskFilter.blur(BlurStyle.normal, 3));
      
      canvas.rotate(engine.time * 3);
      final pSweep = Paint()..shader = ui.Gradient.sweep(Offset.zero, [const Color(0xFF00FF88), Colors.white, const Color(0xFF00FF88)], [0.0, 0.5, 1.0]);
  ...
  @override bool shouldRepaint(covariant GamePainterA1Forest old) => true;
}

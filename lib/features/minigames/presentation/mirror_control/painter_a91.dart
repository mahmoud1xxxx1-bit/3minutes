...
class GamePainterA91 extends CustomPainter {
  GamePainterA91({required this.engine, this.images});
...
// deleted
    ...
    // ==========================================
    canvas.drawRect(const Rect.fromLTWH(0, 0, GameEngine.fieldSize, GameEngine.fieldSize), Paint()..color = const Color(0xFF0F5A3E));
    canvas.save();
    canvas.translate(GameEngine.fieldSize / 2, GameEngine.fieldSize / 2);
    canvas.rotate(engine.time * 0.2);
    for (int i = 0; i < 36; i++) {
      canvas.drawArc(Rect.fromCenter(center: Offset.zero, width: GameEngine.fieldSize * 1.5, height: GameEngine.fieldSize * 1.5),
          i * (math.pi / 18), math.pi / 18, true, Paint()..color = i % 2 == 0 ? const Color(0xFF990000) : const Color(0xFF111111));
    }
    canvas.drawCircle(Offset.zero, GameEngine.fieldSize / 1.8, Paint()..color = const Color(0xFF0F5A3E));
    canvas.restore();
    ...
    for (int i = 0; i < engine.obstacles.length; i++) {
        ...
        RRect rrect = RRect.fromRectAndRadius(obs, const Radius.circular(8));
        canvas.drawRRect(rrect, Paint()..color=const Color(0xFFD4AF37));
        canvas.drawRRect(rrect.deflate(4), Paint()..color=Colors.white);
        for(double y=obs.top+10; y<obs.bottom; y+=40) {
            canvas.drawLine(Offset(obs.left, y), Offset(obs.right, y), Paint()..color=const Color(0xFFD4AF37)..strokeWidth=2);
        }
    }
    ...
    for (int i = 0; i < engine.targets.length; i++) {
        ...
        canvas.drawCircle(target, 15, Paint()..color=const Color(0xFFDDDDDD));
        canvas.drawCircle(target + const Offset(-4, -4), 5, Paint()..color=Colors.white);
        canvas.restore();
    }
  ...
  @override bool shouldRepaint(covariant GamePainterA91 old) => true;
}

...
class GamePainterA62 extends CustomPainter {
  GamePainterA62({required this.engine, this.images});
...
// deleted
    ...
    for (int i = 0; i < engine.obstacles.length; i++) {
        ...
        canvas.drawRect(obs, Paint()..color=const Color(0xFF88CCFF)..style=PaintingStyle.stroke..strokeWidth=4);
        canvas.drawRect(obs, Paint()..color=const Color(0xFFBBDDFF).withValues(alpha: 0.5));
    }
    for (int i = 0; i < engine.targets.length; i++) {
        ...
        canvas.drawCircle(target, 15, Paint()..color=Colors.blue);
        canvas.restore();
    }
  ...
  @override bool shouldRepaint(covariant GamePainterA62 old) => true;
}

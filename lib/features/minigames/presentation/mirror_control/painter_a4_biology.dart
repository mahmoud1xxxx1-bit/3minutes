...
class GamePainterA4Biology extends CustomPainter {
  GamePainterA4Biology({required this.engine, this.images});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // BACKGROUND
    final bgPaint = Paint()..shader = ui.Gradient.linear(const Offset(0, 0), const Offset(0, GameEngine.fieldSize), [const Color(0xFF11001A), const Color(0xFF220033)]);
    ...
    for (int i = 0; i < 30; i++) {
      ...
      double pSize = 3.0 + (i % 4).toDouble();
      canvas.drawCircle(Offset(px, py), pSize, sporeBg);
      canvas.drawCircle(Offset(px, py), pSize*0.3, sporeFg);
    }
    ...
    for (int i = 0; i < engine.obstacles.length; i++) {
      ...
      // Top Face
      final topPaint = Paint()..shader = ui.Gradient.linear(obs.topCenter, obs.bottomCenter, [const Color(0xFF440044), const Color(0xFF220022)]);
    ...
    for (int i = 0; i < engine.targets.length; i++) {
      ...
      if (images != null && images!['target'] != null) {
        ...
        canvas.drawCircle(target + Offset(0, floatOffset), tr, Paint()..color = isNext ? Colors.purpleAccent : Colors.grey);
      }
    }
  ...
  @override bool shouldRepaint(covariant GamePainterA4Biology old) => true;
}

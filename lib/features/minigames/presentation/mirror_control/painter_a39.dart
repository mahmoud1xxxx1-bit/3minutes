...
class GamePainterA39 extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for(int i=0; i<60; i++) {
        canvas.drawCircle(Offset(rnd.nextDouble()*GameEngine.fieldSize, rnd.nextDouble()*GameEngine.fieldSize), 3, Paint()..color=const Color(0xFF7E57C2).withValues(alpha: 0.5));
    }
    ...
    for (int i = 0; i < engine.targets.length; i++) {
      ...
      if (isNext) {
          ...
          canvas.restore();
          canvas.drawCircle(Offset.zero, tr*2.5, Paint()..shader = ui.Gradient.radial(Offset.zero, tr*2.5, [const Color(0xFFFFFF00).withValues(alpha: 0.6), Colors.transparent]));
      } else {
          // Acorn cup / empty

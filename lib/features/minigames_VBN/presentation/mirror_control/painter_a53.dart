...
class GamePainterA53 extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for(int i = 0; i < 15; i++) {
      ...
      canvas.drawCircle(Offset(fx, fy), 20 + rnd.nextDouble()*30, Paint()..color=Colors.purpleAccent.withOpacity(0.05)..maskFilter=const MaskFilter.blur(BlurStyle.normal, 3));
    }
    ...
    for (int i = 0; i < engine.targets.length; i++) {
        ...
        if (isActive) {
            ...
            canvas.drawCircle(target, 30*p, Paint()..color=Colors.deepOrangeAccent.withOpacity(0.6)..maskFilter=const MaskFilter.blur(BlurStyle.normal, 3));
            canvas.drawCircle(target, 15*p, Paint()..color=Colors.yellow.withOpacity(0.8)..maskFilter=const MaskFilter.blur(BlurStyle.normal, 5));
        }
    ...
    if (engine.exitGate != null) {
        canvas.drawOval(Rect.fromCenter(center: engine.exitGate!, width: 60, height: 90), Paint()..color=Colors.teal.withOpacity(0.3)..maskFilter=const MaskFilter.blur(BlurStyle.normal, 3));
